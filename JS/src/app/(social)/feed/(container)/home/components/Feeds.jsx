...
import { CreatePostData,GetPostData,DeletePostData, GetUsersData, getFeedUsers } from '../../../../../api/ApiService';
const ActionMenu = ({
  name
}) => {
...
const Feeds = () => {
  ...
  {/* <h2>Create a Note</h2> */}
      ...
      {noteData?.slice().reverse().map((post, idx) => {
        const postUser = usersData.find(user => user.id === post.author);
        
        return <PostCard 
        createdAt={"osdseffc"}
        likesCount={10}
        image={postUser?.profile_picture}
