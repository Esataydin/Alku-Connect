...
import { getFeedUsers, CreatePostData } from '../../app/api/ApiService'; // getFeedUsers eklendi
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('getFeedUsers response:', response);
      } catch (error) {
...
/>
          ...
          <input
                    ...
                    value={title}
                />
            <textarea className="form-control pe-4 border-0" rows={2} data-autoresize placeholder="Share your thoughts..." 
            id="content"
            name="content"
            value={content}
